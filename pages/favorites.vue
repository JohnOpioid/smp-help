  ...
  <div class="container mx-auto px-4 py-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div v-else class="space-y-4">
        <div v-for="item in favorites" :key="(item as any).id" 
          ...
          <div class="flex items-start justify-between">
            <div class="flex-1">
              <h3 class="font-medium text-slate-900 dark:text-white mb-1">{{ (item as any).title }}</h3>
              <p class="text-sm text-slate-600 dark:text-slate-300">{{ (item as any).description }}</p>
              <div class="flex items-center gap-2 mt-2">
                <span class="text-xs px-2 py-1 bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 rounded">
                  {{ (item as any).type }}
                </span>
                <span v-if="(item as any).category" class="text-xs px-2 py-1 bg-blue-100 dark:bg-blue-900 text-blue-700 dark:text-blue-300 rounded">
                  {{ (item as any).category }}
                </span>
              </div>
            </div>
            <button @click="removeFromFavorites((item as any).id)" 
...
const removeFromFavorites = (id: string) => {
  const index = favorites.value.findIndex((item: any) => item.id === id)
  if (index > -1) {
