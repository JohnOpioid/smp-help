  ...
  <div class="flex-1">
    ...
    <div class="max-w-5xl w-full mx-auto px-2 md:px-4 pt-8">
      <div class="flex items-center justify-between gap-2 mb-2 mt-6">
        ...
        <div class="flex items-center gap-2">
          <UButton 
            ...
            @click="toggleBookmark('gcs-pediatric')"
      ...
      <p class="text-slate-600 dark:text-slate-300">
        Шкала комы Глазго для детей до года. Используется для оценки степени нарушения сознания у младенцев.
      </p>
            ...
            <div class="flex flex-col divide-y divide-slate-100 dark:divide-slate-700">
              <UButton
                ...
                :class="[
                  ...
                  selectedEye === opt.value ? [eyeButtonBgClass, eyeButtonHoverClass] : 'hover:bg-slate-100 dark:hover:bg-slate-700'
                ]"
                :color="'neutral'"
                variant="ghost"
          ...
          <div class="h-full flex flex-col">
            <div class="flex flex-col divide-y divide-slate-100 dark:divide-slate-700">
              <UButton
                ...
                :class="[
                  'w-full justify-between text-left rounded-none !px-4 !py-3 bg-transparent border-0 cursor-pointer',
                  selectedVerbal === opt.value ? [verbalButtonBgClass, verbalButtonHoverClass] : 'hover:bg-slate-100 dark:hover:bg-slate-700'
                ]"
                :color="'neutral'"
                variant="ghost"
          ...
          <div class="h-full flex flex-col">
            <div class="flex flex-col divide-y divide-slate-100 dark:divide-slate-700">
              <UButton
                ...
                :class="[
                  'w-full justify-between text-left rounded-none !px-4 !py-3 bg-transparent border-0 cursor-pointer',
                  selectedMotor === opt.value ? [motorButtonBgClass, motorButtonHoverClass] : 'hover:bg-slate-100 dark:hover:bg-slate-700'
                ]"
                :color="'neutral'"
                variant="ghost"
...
// Критерии для детей до года
const eyeOptions = [
  { value: 4, text: 'Спонтанное открытие глаз' },
  { value: 3, text: 'Открытие глаз на звук' },
  { value: 2, text: 'Открытие глаз на боль' },
  { value: 1, text: 'Глаза не открываются' }
]

const verbalOptions = [
  { value: 5, text: 'Плачет, улыбается, лепечет' },
  { value: 4, text: 'Плачет, но успокаивается при прикосновении' },
  { value: 3, text: 'Беспокойный, раздражительный плач' },
  { value: 2, text: 'Стоны, нечленораздельные звуки' },
  { value: 1, text: 'Отсутствие звуков' }
]

const motorOptions = [
  { value: 6, text: 'Спонтанные движения' },
  { value: 5, text: 'Отдергивание при прикосновении' },
  { value: 4, text: 'Отдергивание при боли' },
  { value: 3, text: 'Аномальное сгибание (декортикация)' },
  { value: 2, text: 'Аномальное разгибание (децеребрация)' },
...
function getScoreInterpretation() {
  if (totalScore.value >= 13) return 'Ясное сознание'
  if (totalScore.value >= 9) return 'Сопор'
  if (totalScore.value >= 4) return 'Кома'
  return 'Глубокая кома'
}
...
onMounted(() => {
  updateIsBookmarked('gcs-pediatric')
})
</script>
