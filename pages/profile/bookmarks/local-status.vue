<template>
  <div>
    ...
    <div v-else-if="bookmarks.length === 0" class="p-6 text-center">
      ...
      <p class="text-slate-600 dark:text-slate-300 mb-2">У вас пока нет закладок локальных статусов</p>
      <p class="text-sm text-slate-500 dark:text-slate-400">Добавляйте интересные локальные статусы в закладки для быстрого доступа</p>
    </div>
    ...
    <div v-else class="bg-white dark:bg-slate-800 rounded-lg overflow-hidden">
      <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center gap-2">
        <UIcon name="i-heroicons-clipboard-document-list" class="w-4 h-4 text-slate-500" />
        <p class="text-sm text-slate-600 dark:text-slate-300">Локальные статусы</p>
      </div>
...
async function loadBookmarks() {
  try {
    ...
    const res: any = await $fetch('/api/bookmarks', {
      query: { _t: Date.now(), type: 'local-status' }
    })
    if (res?.success) {
      bookmarks.value = (res.items || []).filter((b: any) => b.type === 'local-status')
      error.value = null
    } else {
