    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg overflow-hidden">
      <div class="p-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
          <UButton icon="i-heroicons-chevron-left" variant="ghost" color="neutral" size="sm"
            class="w-8 h-8 p-0 cursor-pointer inline-flex items-center justify-center" @click="goPrevMonth"
            aria-label="Предыдущий месяц" />
          <button type="button"
            class="text-sm font-medium text-slate-700 dark:text-slate-200 text-center h-8 rounded hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer inline-flex items-center justify-center"
            @click="goToCurrentMonth" aria-label="Текущий месяц">
            {{ monthLabel }}
          </button>
        ...
        <div class="flex md:items-center gap-2">
          ...
          <UButton :variant="editMode ? 'solid' : 'ghost'" :color="editMode ? 'primary' : 'neutral'" size="sm"
            class="w-8 h-8 p-0 cursor-pointer inline-flex items-center justify-center" @click="toggleEditMode"
      ...
      <div v-if="editMode" class="p-4 border-b border-slate-100 dark:border-slate-700 bg-blue-50 dark:bg-blue-900/20">
        <div class="flex items-center gap-3">
          ...
          <div class="flex-1 relative template-popover-container">
            ...
            <!-- Popover -->
            <div v-if="templatePopoverOpen"
              class="absolute top-full left-0 mt-1 z-50 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-md shadow-lg min-w-[220px]">
                ...
                <div v-else>
                  <button v-for="template in templates" :key="template._id" type="button"
                    ...
                    <span class="truncate">{{ template.title || 'Без названия' }} · {{ template.startTime?.slice(0, 5) }}
                      – {{
                        template.endTime?.slice(0,5) }}</span>
                  </button>
          ...
          <div class="flex flex-wrap gap-2">
            <div v-for="t in templates" :key="t._id" class="inline-flex items-center gap-1">
              <UButton size="xs" variant="soft" color="neutral" class="cursor-pointer"
                @click="(newShiftStartTime = t.startTime, newShiftEndTime = t.endTime || '', openAddShift())">
                {{ (t.startTime || '').slice(0, 5) }}<template v-if="t.endTime"> – {{ (t.endTime || '').slice(0, 5)
                  }}</template>
              </UButton>
              <UButton size="xs" variant="ghost" color="error" icon="i-heroicons-trash" class="cursor-pointer"
                @click="deleteTemplate(t._id)" />
            </div>
          </div>
        </div>
        <!-- Дни недели -->
        ...
        <div class="grid grid-cols-7 gap-0">
          <div v-for="(day, i) in calendarDays" :key="day.key" :class="[
            ...
            <div v-if="getAlternationPreviewFor(day.dateStr) && !alternationRangeEnd"
              ...
              <span :style="getPreviewDotStyle(getAlternationPreviewFor(day.dateStr)?.color)"
                class="block w-2.5 h-2.5 rounded-full"></span>
            </div>
          </div>
        </div>
      ...
      <USlideover v-model:open="templatesSlideoverOpen" side="right" title="Шаблоны смен"
          ...
          <div class="space-y-4 flex-1">
            ...
            <div v-if="activeTemplatesTab === 'alternations' && !alternationOpen">
              ...
              <div v-else class="space-y-2" :key="`alternations-list-${alternations.length}`">
                <div v-for="(a, idx) in alternations" :key="`alternation-${a._id}-${idx}`"
                  ...
                  <div class="flex items-center gap-3 min-w-0">
                    <div class="min-w-0">
                      ...
                      <div class="mt-1 flex flex-wrap gap-1">
                        <div
                          v-for="(d, idx) in (((a.days || []) as Array<any>).filter((x: any) => x && (x.templateId || x.free)))"
                          :key="idx" class="flex items-center justify-center">
                          <span v-if="d.templateId"
                            :style="{ backgroundColor: (templates.find(t => t._id === d.templateId)?.color) || '#3B82F6' }"
                            class="w-2.5 h-2.5 rounded-full"></span>
                  ...
                  <div class="flex items-center gap-2">
                    <UButton size="xs" variant="ghost" color="error" icon="i-heroicons-trash" class="cursor-pointer"
                      @click.stop="deleteAlternation(a._id)"></UButton>
                  </div>
                </div>
              </div>
              ...
              <div class="border border-slate-200 dark:border-slate-700 rounded-md overflow-hidden">
                <div v-for="week in 2" :key="week" class="grid grid-cols-7">
                  <button v-for="d in 7" :key="(week - 1) * 7 + d" type="button"
                    ...
                    <span v-else-if="alternationDays[(week - 1) * 7 + (d - 1)] && alternationDays[(week - 1) * 7 + (d - 1)].free"
                      class="w-4 h-4 rounded-full bg-slate-300 dark:bg-slate-600"></span>
                ...
                <div class="grid grid-cols-2 gap-2">
                  <button v-for="t in templates" :key="t._id" type="button"
                    ...
                    <span :style="{ backgroundColor: (t as any).color || '#3B82F6' }"
                      class="w-3 h-3 rounded-full flex-shrink-0"></span>
                    <div class="flex flex-col leading-tight min-w-0 text-left">
                      <span class="text-xs font-medium truncate">{{ t.title || 'Без названия' }}</span>
                      <span class="text-[11px] text-slate-500 truncate">{{ (t.startTime || '').slice(0, 5) }}<template
                          v-if="t.endTime"> – {{ (t.endTime || '').slice(0, 5) }}</template></span>
                    </div>
                  </button>
                </div>
              </div>
                ...
                <div v-for="(t, idx) in templates" :key="`template-${t._id}-${idx}`"
                  ...
                  <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                      ...
                      <p class="text-xs text-slate-500">{{ (t.startTime || '').slice(0, 5) }}<template v-if="t.endTime">
                          – {{ (t.endTime || '').slice(0, 5) }}</template>
                      </p>
                    </div>
                  </div>
                  <div class="flex items-center gap-2">
                    <UButton size="xs" variant="ghost" color="error" icon="i-heroicons-trash" class="cursor-pointer"
                      @click.stop="deleteTemplate(t._id)" />
                  </div>
                </div>
              </template>
              <template v-else-if="!templateEditorOpen">
            ...
            <div class="space-y-3" v-if="templateEditorOpen && activeTemplatesTab === 'templates'">
              ...
              <div class="flex items-end gap-3">
                ...
                <UPopover>
                  <UButton aria-label="Выбрать цвет" color="neutral" variant="outline" size="lg"
                    class="shrink-0 w-10 h-10 p-0 rounded-md cursor-pointer inline-flex items-center justify-center">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <UFormField label="Время начала" class="w-full">
                  <UPopover v-model:open="templateStartOpen"
                    :content="{ side: 'bottom', align: 'start', sideOffset: 8 }"
                    @update:open="(v: boolean) => { if (!v) applyTemplateStartTime() }">
                    <UInput v-model="templateForm.startTime" placeholder="3 октября" size="xl" readonly
                      class="w-full cursor-pointer" />
                    <template #content>
                      <div class="p-4 w-[260px] select-none" @mousedown.prevent>
                        <div class="flex items-center justify-center gap-3">
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevHour(templateTempStartTime || templateForm.startTime) }}</div>
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextHour(templateTempStartTime || templateForm.startTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">часы</div>
                          </div>
                          <div class="text-[28px] font-extrabold">:</div>
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevMinute(templateTempStartTime || templateForm.startTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextMinute(templateTempStartTime || templateForm.startTime) }}</div>
                ...
                <UFormField label="Время окончания" class="w-full">
                  <UPopover v-model:open="templateEndOpen" :content="{ side: 'bottom', align: 'start', sideOffset: 8 }"
                    @update:open="(v: boolean) => { if (!v) applyTemplateEndTime() }">
                    <UInput v-model="templateForm.endTime" placeholder="3 октября" size="xl" readonly
                      class="w-full cursor-pointer" />
                    <template #content>
                      <div class="p-4 w-[260px] select-none" @mousedown.prevent>
                        <div class="flex items-center justify-center gap-3">
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevHour(templateTempEndTime || templateForm.endTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextHour(templateTempEndTime || templateForm.endTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">часы</div>
                          </div>
                          <div class="text-[28px] font-extrabold">:</div>
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevMinute(templateTempEndTime || templateForm.endTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextMinute(templateTempEndTime || templateForm.endTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">минуты</div>
                          </div>
                        </div>
                      </div>
        ...
        <template #footer>
          <div v-if="alternationOpen" class="flex items-center justify-between gap-2 w-full">
            <UButton size="xl" variant="ghost" color="neutral" icon="i-heroicons-arrow-left"
              class="cursor-pointer w-10 h-10 p-0 inline-flex items-center justify-center"
              @click="alternationOpen = false" />
            <UButton size="xl" color="primary" class="cursor-pointer flex-1 justify-center" @click="saveAlternation">
              Сохранить</UButton>
          </div>
          <div v-else class="flex items-center justify-end gap-2 w-full">
            <template v-if="templateEditorOpen">
              ...
              <UButton size="xl" color="primary" class="cursor-pointer flex-1 justify-center" @click="saveTemplate">
                Сохранить</UButton>
            </template>
            <template v-else>
              <UButton size="xl" color="primary" class="w-full justify-center cursor-pointer px-3 py-2"
                @click="activeTemplatesTab === 'templates' ? startCreateTemplate() : openAlternationSetup()">Добавить
              </UButton>
            </template>
          </div>
        </template>
      ...
      <template v-if="!isMobile">
        <UModal v-model:open="addModalOpen" title="Новая смена" description="Выберите дату и время смены"
          :ui="{ overlay: 'bg-slate-700/50', wrapper: 'sm:max-w-md', content: 'sm:rounded-md rounded-t-md overflow-visible', body: 'p-4 sm:p-6 overflow-visible', close: 'cursor-pointer' }"
          ...
          <template #body>
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <UFormField label="Время начала" class="w-full">
                  <UPopover v-model:open="startTimeOpen" :content="{ side: 'bottom', align: 'end', sideOffset: 8 }"
                    @update:open="(v: boolean) => { if (!v) applyStartTime() }">
                    <UInput v-model="startInputText" size="xl"
                      ...
                      class="w-full" @focus="onStartFocus" @input="onStartTextInput" @blur="onStartInputBlur"
                    ...
                    <template #content>
                      <div class="p-4 w-[260px] select-none" @mousedown.prevent>
                        <div class="flex items-center justify-center gap-3">
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevHour(tempStartTime || newShiftStartTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                              ...
                              @touchmove.prevent="onTimeTouchMove($event)" @touchend.prevent="onTimeTouchEnd">
                              {{ getHours(tempStartTime || newShiftStartTime) }}
                            </div>
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextHour(tempStartTime || newShiftStartTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">часы</div>
                          </div>
                          <div class="text-[28px] font-extrabold">:</div>
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevMinute(tempStartTime || newShiftStartTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextMinute(tempStartTime || newShiftStartTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">минуты</div>
                          </div>
                        </div>
                        <div class="mt-4 grid grid-cols-2 gap-2">
                          <UButton size="sm" variant="soft" color="neutral" @click="setTimeQuick('start', '08:00')">08:00
                          </UButton>
                          <UButton size="sm" variant="soft" color="neutral" @click="setTimeQuick('start', '20:00')">20:00
                          </UButton>
                        </div>
                ...
                <UFormField label="Время окончания" class="w-full">
                  <UPopover v-model:open="endTimeOpen" :content="{ side: 'bottom', align: 'end', sideOffset: 8 }"
                    @update:open="(v: boolean) => { if (!v) applyEndTime() }">
                    <UInput v-model="endInputText" size="xl"
                      ...
                      class="w-full" @focus="onEndFocus" @input="onEndTextInput" @blur="onEndInputBlur"
                      @keydown.enter.prevent="($event.target as HTMLInputElement).blur()" />
                    <template #content>
                      <div class="p-4 w-[260px] select-none" @mousedown.prevent>
                        <div class="flex items-center justify-center gap-3">
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevHour(tempEndTime || newShiftEndTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextHour(tempEndTime || newShiftEndTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">часы</div>
                          </div>
                          <div class="text-[28px] font-extrabold">:</div>
                          <div class="flex-1 text-center">
                            <div class="text-xs text-slate-400 h-5 flex items-end justify-center">{{
                              getPrevMinute(tempEndTime || newShiftEndTime) }}</div>
                            <div class="text-[34px] leading-none font-extrabold cursor-ns-resize"
                            ...
                            <div class="text-xs text-slate-400 h-5 flex items-start justify-center">{{
                              getNextMinute(tempEndTime || newShiftEndTime) }}</div>
                            <div class="text-[11px] text-slate-500 mt-1">минуты</div>
                          </div>
                        </div>
                        <div class="mt-4 grid grid-cols-2 gap-2">
                          <UButton size="sm" variant="soft" color="neutral" @click="setTimeQuick('end', '08:00')">08:00
                          </UButton>
                          <UButton size="sm" variant="soft" color="neutral" @click="setTimeQuick('end', '20:00')">20:00
                          </UButton>
                        </div>
              ...
              <div class="mt-2">
                <div class="flex items-center justify-between mb-1">
                  ...
                  <UButton size="xs" variant="ghost" color="neutral" icon="i-heroicons-plus" class="cursor-pointer"
                    @click="saveCurrentAsTemplate">Сохранить текущие</UButton>
                </div>
                <UPopover>
                  <UButton size="xl" variant="soft" color="neutral" class="w-full cursor-pointer">Выбрать из шаблона
                  </UButton>
                  <template #content>
                    <div class="p-2 space-y-1">
                      ...
                      <div v-else class="space-y-1">
                        <button v-for="t in templates" :key="t._id" type="button"
                          class="w-full text-left px-2 py-2 rounded hover:bg-slate-100 dark:hover:bg-slate-800/60 text-xs flex items-center gap-2 cursor-pointer"
                          @click="applyTemplate(t)">
                          <span :style="{ backgroundColor: (t as any).color || '#3B82F6' }"
                            class="w-2.5 h-2.5 rounded-full"></span>
                          <span class="truncate"><template v-if="t.title">{{ t.title }} · </template>{{ (t.startTime ||
                            '').slice(0,5) }}<template v-if="t.endTime"> – {{ (t.endTime || '').slice(0, 5)
                              }}</template></span>
                        </button>
                      </div>
                    </div>
          ...
          <template #footer>
            <div class="flex gap-3 w-full">
              <UButton v-if="editingShiftId" color="error" variant="soft" size="xl" type="button"
                @click="deleteCurrentShift" class="cursor-pointer flex-1 justify-center items-center">Удалить</UButton>
              <UButton color="primary" :disabled="!canSaveShift" size="xl" @click="addShift"
                class="cursor-pointer flex-1 justify-center items-center">Сохранить</UButton>
            </div>
          </template>
        </UModal>
      </template>
      ...
      <template v-else>
        <ClientOnly>
          <BottomSheet v-model="addModalOpen" title="Новая смена" @close="addModalOpen = false">
            <div class="p-4 space-y-3">
              ...
              <div>
                ...
                <div v-if="selectedDateShifts.length > 0" class="space-y-1">
                  <div v-for="s in selectedDateShifts" :key="s._id"
                    class="flex items-center justify-between text-xs px-2 py-1 rounded bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300">
                    <span class="truncate">{{ s.title }}</span>
                    <UButton color="error" variant="ghost" size="xs" icon="i-heroicons-trash" class="cursor-pointer"
                      @click="deleteShift(s._id)" />
                  </div>
                </div>
              ...
              <div class="mt-6">
                <div class="flex gap-3 w-full">
                  <UButton v-if="editingShiftId" color="error" variant="soft" size="xl" type="button"
                    @click="deleteCurrentShift" class="cursor-pointer flex-1 justify-center items-center">Удалить
                  </UButton>
                  <UButton color="primary" :disabled="!canSaveShift" size="xl" @click="addShift"
                    class="cursor-pointer flex-1 justify-center items-center">Сохранить</UButton>
                </div>
