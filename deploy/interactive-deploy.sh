...
EOF
    ...
    # Собираем проект с улучшенной обработкой ошибок
    log "🔨 Собираем проект..."
    log "📋 Это может занять несколько минут (до 30 минут)..."
    
    # Проверяем доступную память
    FREE_MEM=$(free -m | awk 'NR==2{printf "%.0f", $4}' 2>/dev/null || echo "0")
    if [ "$FREE_MEM" != "0" ] && [ "$FREE_MEM" -lt 2048 ]; then
        warn "⚠️  Внимание: доступно менее 2GB памяти (${FREE_MEM}MB). Сборка может быть медленной."
    fi
    
    # Очищаем предыдущие сборки для освобождения места
    log "🧹 Очищаем предыдущие сборки..."
    rm -rf .nuxt .output 2>/dev/null || true
    
    # Запускаем сборку с таймаутом (30 минут) и перенаправлением вывода
    log "🚀 Запускаем сборку с таймаутом 1800 секунд..."
    if timeout 1800 npm run build:production 2>&1 | tee /tmp/build.log; then
        BUILD_EXIT_CODE=${PIPESTATUS[0]}
        if [ "$BUILD_EXIT_CODE" -eq 124 ]; then
            error "❌ Сборка прервана по таймауту (30 минут)"
            error "❌ Проверьте логи: tail -100 /tmp/build.log"
            exit 1
        fi
...
main() {
    case $INSTALL_MODE in
        ...
        "quick-update")
            ...
            if timeout 1800 npm run build:production 2>&1 | tee /tmp/build-update.log; then
                BUILD_EXIT_CODE=${PIPESTATUS[0]}
                if [ "$BUILD_EXIT_CODE" -eq 124 ]; then
                    error "❌ Сборка прервана по таймауту (30 минут)"
                    error "❌ Проверьте логи: tail -100 /tmp/build-update.log"
                    exit 1
                fi
                log "✅ Проект собран успешно"
            else
