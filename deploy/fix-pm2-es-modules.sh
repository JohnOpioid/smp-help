...
log "🔧 Исправляем проблему с PM2 и ES модулями..."
...
if [ -f "package.json" ]; then
    ...
    # Проверяем, есть ли type: "module"
    if grep -q '"type": "module"' package.json; then
        log "✅ package.json содержит type: module"
    else
        log "⚠️ package.json не содержит type: module"
    fi
...
# Создаем правильную конфигурацию PM2 для ES модулей
