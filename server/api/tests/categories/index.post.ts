...
export default defineEventHandler(async (event) => {
  ...
  const { name, description, courseLink, isPublic } = body || {}
  ...
  const item = await TestCategory.create({ 
    name, 
    url, 
    description, 
    courseLink: typeof courseLink === 'string' ? String(courseLink).trim() : undefined,
