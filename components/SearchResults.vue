    ...
    <div class="max-w-5xl mx-auto px-4 py-8">
      ...
      <div v-else class="text-center py-12">
        <div class="text-slate-400 dark:text-slate-500">
          ...
          <p class="text-lg font-medium mb-2">Ничего не найдено</p>
          <p class="text-sm">Попробуйте изменить запрос или использовать другие ключевые слова</p>
        </div>
      </div>
    </div>
  </div>
...
const renderAlgorithmTable = (content: string): string => {
  ...
  table = table.replace(/<th[^>]*>/gi, '<th class="px-3 py-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300 text-center font-medium whitespace-normal break-words align-middle sticky top-0 z-20 bg-slate-200 dark:bg-slate-800 sm:whitespace-normal whitespace-nowrap">')
...
:deep(.dark table tbody tr:nth-child(even)) {
...
/* Стили для мобильных таблиц с data-styled-table-wrapper */
/* Убираем внешние бордеры у таблицы */
:deep([data-styled-table-wrapper] table) {
  border: 0;
}

/* Вертикальные внутренние бордеры */
...
/* Горизонтальные внутренние бордеры тем же цветом, что и вертикальные */
:deep([data-styled-table-wrapper] table thead tr th),
:deep([data-styled-table-wrapper] table tbody tr td) {
  border-top: 1px solid rgb(241 245 249);
  border-bottom: 1px solid rgb(241 245 249);
}
.dark :deep([data-styled-table-wrapper] table thead tr th),
.dark :deep([data-styled-table-wrapper] table tbody tr td) {
  border-top-color: rgb(51 65 85);
  border-bottom-color: rgb(51 65 85);
}

/* Заголовки таблицы в одну строку с обрезкой */
:deep([data-styled-table-wrapper] thead th) {
  white-space: nowrap;
  overflow: hidden;
